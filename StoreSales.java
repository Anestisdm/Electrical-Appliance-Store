			...
			while (line.isEmpty());
				if (line.trim().equalsIgnoreCase("SALES_LIST")) {
					...
					while (line.isEmpty());
							...
							while(!line.trim().equals("}")) {
								...
								if (line.trim().equals("SALE")) {
									...
									while (!line.trim().equals("{"));
											...
											if (x==false) {
												System.out.println("Το προιόν "+Model_Name+" του κατεσκυαστή "+Manufacturer+" δεν μπορεί να προστεθεί στον κατάλογο των πωλήσεων, γιατι δεν βρίσκεται στον κατάλογο των προιόντων! ");
											}	
										}
										line = reader.readLine();
									}
						...
						System.out.println("Το αρχείο "+Sales+" είναι κενό!");
					}
				}
}
