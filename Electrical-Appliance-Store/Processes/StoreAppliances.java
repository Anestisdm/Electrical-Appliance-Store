	...
	public void LoadFile (String Products) {
		...
		try {
			...
			if (line.trim().equalsIgnoreCase("ITEM_LIST")){
									...
									while (!line.trim().equals("{"));
											...
											if (Code!=null && Item_Type!=null && Model_Name!=null) {
												if (Item_Type.equalsIgnoreCase("tv")) {
													...
													item = new Console(Code,Model_Name,Model_year,Manufacturer,Price,Pieces,Type,Processor,Graphics,Sound,Capacity_of_HDD);
												}
												...
												System.out.println("Για να ολοκληρωθεί η καταχώρηση του προιόντος θα πρέπει να λαμβάνουν τιμές τα πεδία CODE, ITEM_TYPE και MODEL! ");
											}	
									}
									line = reader.readLine();
								}
				...
				System.out.println("Το αρχείο "+Products+" είναι κενό!");
			}
		}
}
